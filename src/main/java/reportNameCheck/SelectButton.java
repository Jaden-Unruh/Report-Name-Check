...
class SelectButton extends JButton {
	SelectButton(final int whichSelect, final boolean isDirs) {
		super("Select...");
				...
				SwingWorker<Boolean, Void> sw = new SwingWorker<Boolean, Void>() {
					...
					protected Boolean doInBackground() throws Exception {
						...
						if (returnVal == JFileChooser.APPROVE_OPTION) {
							...
							Main.selectedFiles[whichSelect] = selectedFile;
							rename(selectedFile.getName());
							
							if (whichSelect == 1) {
								Main.open.setEnabled(true);
								String preview = "";
								for(File file : selectedFile.listFiles())
									preview += file.getName() + "\n";
								Main.preview.setText(preview);
							}
